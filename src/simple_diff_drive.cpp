...
public:
    ...
    {
        sub_for_left_counts_ = this->create_subscription<std_msgs::msg::Int16>(
            "left_wheel", 1000, std::bind(&SimpleDiffDrive::calc_left_vel, this, std::placeholders::_1));
        sub_for_right_counts_ = this->create_subscription<std_msgs::msg::Int16>(
            "right_wheel", 1000, std::bind(&SimpleDiffDrive::calc_right_vel, this, std::placeholders::_1));
        sub_for_velocity_ = this->create_subscription<geometry_msgs::msg::Twist>(
            "cmd_vel", 1, std::bind(&SimpleDiffDrive::set_speeds, this, std::placeholders::_1));
    }

private:
    rclcpp::Subscription<std_msgs::msg::Int16>::SharedPtr sub_for_right_counts_;
    rclcpp::Subscription<std_msgs::msg::Int16>::SharedPtr sub_for_left_counts_;
    rclcpp::Subscription<geometry_msgs::msg::Twist>::SharedPtr sub_for_velocity_;
    void calc_left_vel(const std_msgs::msg::Int16 &lCount)
    {
        ...
        leftVelocity = cycleDistance / TICKS_PER_M / (rclcpp::Clock(RCL_ROS_TIME).now().seconds() - lastTime);
        lastCount = lCount.data;
        lastTime = rclcpp::Clock(RCL_ROS_TIME).now().seconds();
    }
    void calc_right_vel(const std_msgs::msg::Int16 &rCount)
    {
        ...
        rightVelocity = cycleDistance / TICKS_PER_M / (rclcpp::Clock(RCL_ROS_TIME).now().seconds() - lastTime);
        lastCount = rCount.data;
        lastTime = rclcpp::Clock(RCL_ROS_TIME).now().seconds();
    }
    void set_speeds(const geometry_msgs::msg::Twist &cmdVel)
    {
        lastCmdMsgRcvd = rclcpp::Clock(RCL_ROS_TIME).now().seconds();
...
int main(int argc, char **argv)
{
    ...
    while (rclcpp::ok())
    {
        ...
        if (rclcpp::Clock(RCL_ROS_TIME).now().seconds() - lastCmdMsgRcvd > 1)
        {
            cout << "NOT RECIEVING CMD_VEL - STOPPING MOTORS  --  time sincel last = " << rclcpp::Clock(RCL_ROS_TIME).now().seconds() - lastCmdMsgRcvd << endl;
            leftPwmReq = 0;
            rightPwmReq = 0;
        }
